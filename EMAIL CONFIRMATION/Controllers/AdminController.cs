    ...
    public class AdminController : Controller
    {
        ...
        public async Task<ActionResult> Index(AdminUserViewModel model, string sortOrder, string searchString, string searchRank, int? page, ManageMessageId? message = null)
        {
            ViewBag.StatusMessage =
             message == ManageMessageId.UserDeleted ? "Pomyślnie usunięto konto użytkownika."
             : message == ManageMessageId.UserUpdated ? "Konto użytkownika zostało poyślnie zmodyfikowane."
             : "";

            ViewBag.ErrorMessage =
                message == ManageMessageId.Error ? "Wystąpił błąd skontaktuj się z użytkownikiem."
                : message == ManageMessageId.HighRankedUser ? "Nie masz wystarczających uprawnień by wykonąć tą operację."
                : "";
        ...
        public async Task<ActionResult> ShowUserDetails(AdminUserViewModel model, string sortOrder, string searchString, string searchRank, int? page)
        {
            ...
            foreach (var user in users)
            {
                ...
                model.UserFullName = user.UserName + " " + user.UserName;
            ...
            rlList.Add(new AdminRoleViewModel() { Role = "Zarejestrowany", RoleId = "2", RoleValue = "Zarejestrowany" });
            rlList.Add(new AdminRoleViewModel() { Role = "Potwierdzony", RoleId = "3", RoleValue = "Potwierdzony" });
            rlList.Add(new AdminRoleViewModel() { Role = "Serwisant", RoleId = "4", RoleValue = "Serwisant" });
            rlList.Add(new AdminRoleViewModel() { Role = "Księgowy", RoleId = "5", RoleValue = "Księgowy" });
        ...
        {
            if (AdmUsrRole == "Admin" || AdmUsrRole == "Zarejestrowany")
            {
        ...
        public IEnumerable<SelectListItem> GetUserRoles(string usrrole)
        {
            ...
            rlList.Add(new AdminRoleViewModel() { Role = "Zarejestrowany", RoleId = "2" });
            rlList.Add(new AdminRoleViewModel() { Role = "Potwierdzony", RoleId = "3" });
            rlList.Add(new AdminRoleViewModel() { Role = "Serwisant", RoleId = "4" });
            rlList.Add(new AdminRoleViewModel() { Role = "Księgowy", RoleId = "5" });
